...
@Component({
  selector: 'app-admin-report-to-barangay-information',
  templateUrl: './admin-report-to-barangay-information.component.html',
  styleUrls: ['./admin-report-to-barangay-information.component.css', '../../assets/bootstrap/bootstrap.min.css'],
  providers: [AdminRegistrationService, DatePipe]
})
export class AdminReportToBarangayInformationComponent {
  ...
  ngOnInit() {
    ...
    this.route.queryParams.subscribe(params => {
      ...
      this.customizedReportId = this.reports.reportId;
      this.sender = this.reports.barangay;

      // Fetch user data based on userId.
      this.adminRegistrationService.getAdminData(this.userId).subscribe(
        (data: any) => {
          ...
          (data: any) => {
            if (data && data.adminResponseData) {
              ...
              if (this.mergedResponseData.length > 0) {
                ...
                this.latestResponseStatus = 'New Report'; // Provide a default status if no responses are available
              }
  ...
  respond(form: NgForm) {
    ...
    if (this.isFormValid()) {
      ...
      form.value.reportId = this.reportId.toString();
      ...
      form.value.recipient = this.sender;
      ...
      if (selectedFiles.length > 0) {
        ...
        this.convertFilesToBase64(selectedFiles, (base64Array) => {
          ...
          this.adminRegistrationService.editBarangayReportStatus(this.reportId, form.value).subscribe(
            (response) => {
              console.log('Admin data updated successfully', response, form.value);
            },
            (error) => {
  ...
  openReport(report: any) {
    this.adminRegistrationService.getReportToBarangay(report._id).subscribe(
      (response: any) => {
          this.router.navigateByUrl('/barangay-response-view', { state: { reports: report, customizedReportId: this.customizedReportId, sender: this.sender, userId: this.userId } });
