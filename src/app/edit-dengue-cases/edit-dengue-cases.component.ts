...
@Component({
  selector: 'app-edit-dengue-cases',
  templateUrl: './edit-dengue-cases.component.html',
  styleUrls: ['./edit-dengue-cases.component.css','../../assets/bootstrap/bootstrap.min.css'] ,
  providers: [AdminRegistrationService]
})
export class EditDengueCasesComponent {
  ...
  confirmSave(form: NgForm) {
    ...
    if (this.selectedFile) {
      ...
      this.convertFileToBase64(this.selectedFile, (base64String) => {
        ...
        this.adminService.editDenguePost(this.fetchedProjectData[0]._id, form.value).subscribe(
          (response) => {
            this.isLoading = false;
  ...
  ngOnInit() {
    ...
    this.route.queryParams.subscribe(params => {
      this.projectData = history.state.denguePostData;
      this.projectId = this.projectData._id;

      this.adminService.getDenguPostById(this.projectId).subscribe(
        (response: any) => {
          this.fetchedProjectData = response.denguePostData; // Assign fetched data to userData
