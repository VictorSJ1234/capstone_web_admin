...
@Component({
  selector: 'app-barangay-response-view',
  templateUrl: './barangay-response-view.component.html',
  styleUrls: ['./barangay-response-view.component.css', '../../assets/bootstrap/bootstrap.min.css'],
  providers: [AdminRegistrationService, DatePipe]
})
export class BarangayResponseViewComponent {
  //! means undefined
  userData: any;
  reports: any;
  userId!: string;
  ...
  ngOnInit() {
    this.route.queryParams.subscribe(params => {
      ...
      this.userId = this.reports.userId; 
      this.reportId = this.reports.reportId; 
      this.customizedReportId = history.state.customizedReportId;
      this.sender = history.state.sender;
      this.barangayAdminSenderId = history.state.userId;
      console.log("hello",this.barangayAdminSenderId);
      console.log("hello",this.reportId);
      

      // Fetch user data based on userId.
      this.adminRegistrationService.getAdminData(this.barangayAdminSenderId).subscribe(
        (data: any) => {
    ...
    if (this.isFormValid()) {
      ...
      if (selectedFiles.length > 0) {
        ...
        this.convertFilesToBase64(selectedFiles, (base64Array) => {
          ...
          this.adminRegistrationService.editBarangayReportStatus(this.reports._id, form.value).subscribe(
            (response) => {
              this.openCarouselModalSuccess();
